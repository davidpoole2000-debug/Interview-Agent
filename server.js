...
function renderReportMarkdown({ assessorName, modelName, candidateName, role, coverage, overall0to100, recommendation, result }) {
  const lines = [];
  ...
  lines.push(`**Coverage:** ${coverage.skills_answered}/${coverage.skills_total} skills answered (${Math.round(coverage.fraction * 100)}%)`);
  ...
  for (const s of result.skill_scores || []) {
    ...
    if (s.what_was_missing_for_next_level) lines.push(`- **Missing for next level:** ${s.what_was_missing_for_next_level}`);
...
<body>
  ...
  <div id="exercise" class="card" style="display:none;">
    ...
    <p class="muted">Explain how you’d use AI to summarise a complex document for executives, how you’d validate it, and what you do if it’s confidently wrong.</p>
    <textarea id="exerciseAnswer"></textarea>

    <div style="margin-top:8px;">
      <span class="muted">Auto-exercise:</span>
...
app.post("/api/auto-exercise", async (req, res) => {
  try {
    ...
    const exerciseQ =
      "Explain how you’d use AI to summarise a complex document for executives, how you’d validate it, and what you do if it’s confidently wrong.";

    const prompt = `
Generate a realistic candidate answer (plain text) to this exercise.
...
Rules:
- Include workflow steps, validation approach, and failure handling.
- Higher levels should mention grounding/citations, structured outputs, review loops, monitoring signals.
- Return plain text only.
`.trim();
    ...
    const answer = await callChatText({
      model,
      messages: [
        { role: "system", content: "You are simulating a candidate answer. Clear, structured, not overly long." },
        { role: "user", content: prompt },
      ],
      temperature: 0.8,
    });
...
/* ==========================
   API: Finish (3 assessors: Llama, Qwen, Gemma)
========================== */
